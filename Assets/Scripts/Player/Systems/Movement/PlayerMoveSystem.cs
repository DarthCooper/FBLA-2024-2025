...
partial struct PlayerMoveSystem : ISystem
{
    ...
    {
        foreach (var (velocity, moveInput, jumpInput, sprintInput, moveSpeed, sprintSpeed ,jumpForce, entity) in SystemAPI.Query<RefRW<PhysicsVelocity>, PlayerMoveInput, PlayerJumpInput, PlayerSprintInput, PlayerMoveSpeed, PlayerSprintSpeed, PlayerJumpForce>().WithNone<Stunned>().WithNone<PlayerSpeaking>().WithEntityAccess())
        {
